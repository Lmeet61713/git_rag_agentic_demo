...
| repos | id, user_id, owner, repo, default_branch, last_commit_sha, index_status | 仓库元数据与索引状态 |
| index_jobs | id, repo_id, status, stage, progress, error | 异步入库任务 |
| files | id, repo_id, path, file_type, language, file_hash | 已索引文件清单 |
| chat_sessions | id, user_id, title, created_at | 会话 |
| chat_messages | id, session_id, role, content, sources | 聊天记录 |
...
| GET | /api/repos/{owner}/{repo}/index/status | 入库状态 |
| GET | /api/jobs/{job_id} | 任务详情 |
| GET | /api/search | 直接检索 |
| POST | /api/chat | Agent 问答，SSE 流式返回 |
...
- [x] 将 demo 模式迁移到 MyAgentic 模块（基础完成，ONNX/Chroma 待接入）。
