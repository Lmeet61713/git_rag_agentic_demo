...
- `repo_brief` 仓库列表工具、口语化项目介绍与语言级项目摘要检索。
- Tavily 联网搜索工具：API Key 加密存入后端，搜索失败时明确报错，不静默走本地兜底。
- 路由双重校验：仓库列表/项目介绍只在明确仓库上下文时触发，日常问题交给通用闲聊或联网搜索。
- 联网搜索结果默认折叠，点击展开后查看原始来源卡片。
...
| `repo_brief` | “仓库里有什么/大概有哪些/介绍所有仓库” | 列出仓库名称、一句话简介和主要语言 |
| `web_search` | “联网搜索/最新新闻/实时信息/网上的内容” | 调用 Tavily 搜索并返回来源链接 |
